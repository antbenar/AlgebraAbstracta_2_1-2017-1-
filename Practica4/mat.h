...
vector<int> convertir_bits(ZZ num){
    vector<int>res;
    while(num>0){
        vector.push_back(MOD(num,2));
        num=num/2;
    }
    return res;
}
...
template<typename T>
T exponenciacion(base, vector<int> exponente, T mod)
{
	T resultado = T(1);
	for(int i=exponente.size();i>0;i--) {
...
minum euclides_extendido(minum dividendo,minum divisor){
     vector <minum> cocientes;
     minum residuo,temp,x,y,x_1,y_1;
     x=1;y=0;
     x_1=0;y_1=1;
...
ZZ restoChino(vector<ZZ>a,vector<ZZ>p,int tam){
    ZZ resultado,p_1,q;
    ZZ x_0,p_;
    x_0=0;
    p_=p[0];
    ...
    for(int i=0;i<tam;i++){
        if(p_1[i]>p[i])
            q.push_back(inversa(MOD(p_1[i],p[i]),p[i]));
        else q.push_back(inversa(p_1[i],p[i]));//cout<<q[i]<<endl;
    }
    for(int i=0;i<tam;i++){
        temp=a[i]*p_1[i];
        if (temp>p_)
...
int PHI_euler(int num){
    ...
    vector<int>primo=primos(num);
    int i=0;
    while(i<=num){
        if(MOD(num,primo[i])==0){
            num=num/primo[i];
            exp++;
        }
        else{
    ...
    for(int i=0;i<exp_factores.size();i++){//////////////////////////EMPEZAMOS A CALCULAR EL PHI DE EULER
        ...
        if(exp_factores[i]==1){
            ...
            resultado*=(pow((primo[i]),exp_factores[i])- pow(primo[i],exp_factores[i]-1));
        }
    }
    return resultado;
}
